...
class Scheduler:
    ...
    # # broadcast the end of a task to all managers
    # def _broadcast_end_task(self, task_key: TaskKey, status: TaskStatus):
    #     for manager in self.managers:
    #         manager.end_task(task_key, status)
    #
    # # broadcast the start of a task to all managers
    # def _broadcast_start_task(self, task_key: TaskKey):
    #     for manager in self.managers:
    #         manager.start_task(task_key)
    def monitor_outputs(self, stop_event: threading.Event, task_key: TaskKey, manager: SchedulerEventManager, output_file: str, poll_freq: int = 10):
    ...
    def run(self):
        ...
        while not self.job_state.is_job_complete():
            ...
            image_list_monitor = list(pipeline_output_image_list_env.values())[0]
            ...
            progress_thread_stop_evt = threading.Event()
            thread_args = (progress_thread_stop_evt, current_task_key, self.manager, image_list_monitor, 2)
            progress_thread = threading.Thread(target=self.monitor_outputs,
                                               args=thread_args,
                                               daemon=True)
